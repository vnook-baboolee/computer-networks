...
## 22. Для превращения Ethernet в технологию операторского класса были произведены изменения в нескольких областях, среди них (выберите вариант ответа):</br>а) отменена широковещательная рассылка;</br>б) разделены адресные пространства пользователя и провайдера;</br>в) добавлены функции ОАМ.

Ответ б), в)

**б) Разделены адресные пространства пользователя и провайдера — верно.**
- В классическом Ethernet все устройства находились в одном L2‑домене. Для операторских сетей это неприемлемо по соображениям
безопасности и масштабируемости.
- Решение: технологии **Q‑in‑Q** (802.1ad), **PBB** (Provider Backbone Bridges) и др. создают:
    - отдельное адресное пространство *пользователя* (Customer VLAN, C‑VLAN);
    - изолированное адресное пространство *провайдера* (Service VLAN, S‑VLAN).
- Результат: MAC‑адреса клиентов не видны в ядре сети провайдера; трафик разных клиентов изолирован.
**в) Добавлены функции ОАМ — верно.**
- **ОАМ** (*Operations, Administration, and Maintenance* — эксплуатация, администрирование, обслуживание) — набор механизмов для:
    - мониторинга состояния сети;
    - диагностики неисправностей;
    - измерения параметров (задержка, потеря пакетов);
    - автоматического оповещения о сбоях.
- Примеры стандартов:
    - **IEEE 802.1ag** (Connectivity Fault Management, CFM) — проверка связности;
    - **Y.1731** — расширенные функции OAM для операторских сетей.
- Без этих функций Ethernet не мог бы соответствовать требованиям операторского класса (SLA, быстрое восстановление, отчётность).

**а) Отменена широковещательная рассылка — неверно.**
- Широковещательная рассылка (**broadcast**) — фундаментальный механизм Ethernet, необходимый для работы ряда протоколов 
(например, ARP, DHCP).
- В сетях операторского класса broadcast **не отменяют**, а **контролируют и ограничивают** (через VLAN, private VLAN, фильтрацию).
- Полная отмена сделала бы невозможной базовую работу сети.

## Почему это важно для «операторского класса»
Ethernet изначально создавался для локальных сетей. Чтобы он стал пригоден для услуг провайдеров (VPN, интернет‑доступ, VoIP),
требовалось:
- обеспечить **изоляцию клиентов** (вариант б);
- добавить **инструменты управления и контроля** (вариант в);
- сохранить совместимость с базовыми механизмами (поэтому broadcast не отменяют — вариант а неверен).

---

## 23. Каждая точка МЕР выполняет следующие функции (выберите вариант ответа):</br>а) генерирует сообщения ССМ;</br>б) анализирует транзитные IP-пакеты;</br>в) принимает сообщения ССМ.

Ответ а), в)

- **МЕР** (*Maintenance Entity Point* — точка обслуживания) — логическая сущность в рамках механизмов **OAM** 
(*Operations, Administration, and Maintenance*) для Ethernet (стандарт IEEE 802.1ag).
- **ССМ** (*Continuity Check Message* — сообщение проверки связности) — служебные кадры, которые регулярно рассылаются для:
    - подтверждения работоспособности соединения;
    - обнаружения обрывов или неисправностей.

**а) Генерирует сообщения ССМ — верно.**  
Каждая точка МЕР **активно отправляет** ССМ‑сообщения в рамках своего домена обслуживания. Это позволяет:
- другим точкам МЕР «видеть» её присутствие;
- контролировать целостность пути (если ССМ перестают приходить — фиксируется сбой).
**в) Принимает сообщения ССМ — верно.**  
  Помимо отправки, каждая точка МЕР **обязана принимать** ССМ от других точек своего домена. Это даёт возможность:
- проверять доступность соседей;
- фиксировать потерю связности (если сообщения не приходят в течение заданного времени);
- формировать события для системы мониторинга.

**б) Анализирует транзитные IP‑пакеты — неверно.**  
Точки МЕР работают на **канальном уровне (L2)**. Их задача — контроль Ethernet‑соединений, а не обработка IP‑трафика 
(уровень 3). Анализ IP‑пакетов — функция маршрутизаторов и межсетевых экранов, но не элементов OAM для Ethernet.

### Почему это важно
Механизм ССМ — основа диагностики в операторских сетях Ethernet. Точки МЕР:
- непрерывно «перекликаются» через ССМ;
- мгновенно выявляют сбои;
- обеспечивают выполнение SLA за счёт прозрачности состояния инфраструктуры.
...
Такая конфигурация некорректна. Согласно стандартам CFM (Connectivity Fault Management), уровень домена оператора должен
быть ниже, чем уровень домена пользователя.

В CFM домены организованы иерархически: чем выше уровень домена, тем шире его охват. Рекомендуемое распределение уровней:
* **Домены пользователей (Customer Domain)** — самые крупные, им присваиваются более высокие уровни (например, 7).
* **Домены провайдеров (Provider Domain)** — промежуточные, уровни могут быть, например, 3.
* **Домены операторов (Operator Domain)** — наименьшие, им соответствуют более низкие уровни (например, 1).

Таким образом, если домену оператора назначен уровень 6, а домену пользователя — уровень 3, это нарушает иерархию: домен
пользователя окажется «внутри» домена оператора, что противоречит принципам работы CFM. Домены не должны пересекаться,
а вложенность возможна только при условии, что внешний домен имеет более высокий уровень, чем вложенный.

Если бы уровни были распределены корректно (например, оператору — уровень 1, пользователю — уровень 7), то домен пользователя
мог бы включать в себя домен оператора, и такая конфигурация была бы допустима.

---

## 25. Пограничные коммутаторы провайдера, работающие по стандарту РВВ, должны изучать МАС-адреса узлов клиента (выберите вариант ответа):</br>а) никогда;</br>б) всегда;</br>в) при оказании услуги E-LAN.

Ответ в)

1. **Что такое PBB (Provider Backbone Bridge)**  
   Это стандарт IEEE 802.1ah, позволяющий провайдерам строить «прозрачные» магистральные сети. Ключевая идея: изоляция 
MAC‑адресов клиентов от ядра сети провайдера. В типичном режиме пограничные коммутаторы *не изучают* MAC‑адреса клиентских
узлов — они работают с MAC‑адресами других коммутаторов PBB.
2. **Когда правило меняется: услуга E‑LAN**
    - E‑LAN (Ethernet Virtual Private LAN Service) эмулирует единую локальную сеть для клиента: все его узлы должны «видеть»
   друг друга на канальном уровне (L2).
    - Чтобы обеспечить такую связность, пограничный коммутатор *обязан* изучить MAC‑адреса узлов клиента — иначе он не сможет
   корректно коммутировать кадры между ними.
    - В этом сценарии коммутатор ведёт отдельную таблицу MAC‑адресов для клиентского домена (Customer MAC Table), не смешивая
   её с MAC‑адресами сети провайдера.
3. **Почему не подходят другие варианты**
    - **а) никогда** — неверно: в случае E‑LAN изучение необходимо.
    - **б) всегда** — неверно: по умолчанию в PBB адреса клиентов *не изучаются* (это основа изоляции и масштабируемости).

**Итог:**  
Пограничные коммутаторы PBB изучают MAC‑адреса клиентов **только** при предоставлении услуги E‑LAN, где требуется полная
L2‑связность между узлами клиента. В остальных режимах PBB эта функция отключена.
